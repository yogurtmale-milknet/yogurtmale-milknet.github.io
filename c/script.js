...
function startHorseRacing() {
    ...
    const horsesNames = ['Horse 1', 'Horse 2', 'Horse 3', 'Horse 4'];
    const playerHorseIndex = horsesNames.indexOf(playerHorse);

    // Use correct container from index.html
    const track = document.getElementById('horse-racing');
    track.innerHTML = ''; // Clear previous race
    horsePositions = [0, 0, 0, 0]; // Reset positions

    // Create horse elements
    horses.forEach((horse, index) => {
        const horseLine = document.createElement('div');
        horseLine.classList.add('horse-track-line');

        const horseEmoji = document.createElement('span');
        horseEmoji.classList.add('horse');
        horseEmoji.id = `horse-${index}`;
        horseEmoji.innerText = horse;

        horseLine.appendChild(horseEmoji);
        track.appendChild(horseLine);
    });
    ...
    raceInterval = setInterval(() => {
        ...
        horsePositions = horsePositions.map(pos => pos + (Math.random() * 5) + 2);
        horsePositions.forEach((pos, index) => {
            const horseElement = document.getElementById(`horse-${index}`);
            horseElement.style.left = `${pos}%`;

            if (pos >= 90 && !raceEnded) {
                raceEnded = true;
                const winner = index === playerHorseIndex ? 'You win!' : `Horse ${index + 1} wins!`;
